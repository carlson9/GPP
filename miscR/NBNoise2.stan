...
model {
  ...
  for(n in 1:N){
    ...
    for(n in 1:N) eta[n] ~ normal(yb0 + xz1[n]*b1 + xz2[n]*b2 + xz3[n]*b3 + X[n,]*B + y_year_re[Year[n]] + y_country_re[Country[n]] + y_GP_term[Year[n], Country[n]], .01);
    
    y_in ~ neg_binomial_2_log(eta[y_obs_ind], phi);
}
generated quantities{
  int ystar[y_N_miss];
  for(n in 1:y_N_miss) ystar[n] = neg_binomial_2_log_rng(eta[y_miss_ind[n]], phi);
}
